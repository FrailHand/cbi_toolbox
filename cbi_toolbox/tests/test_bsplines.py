...
def test_convert(sig, degree, tolerance, condition):
    source = sig.copy()
    coeffs = bsplines.convert_to_interpolation_coefficients(
        source, degree, tolerance, boundary_condition=condition, in_place=True
    )
    r_signal = bsplines.convert_to_samples(coeffs, degree, condition, in_place=True)
...
def test_dims(data, from_b, to_b, degree, axes, tolerance, condition):
    ...
    np.testing.assert_allclose(out_1[..., 0], out_2)


def test_dims_conversion(data, degree, tolerance, condition):
    source = data.copy()
    sourceb = data.copy()
    c_1 = bsplines.convert_to_interpolation_coefficients(
        source, degree, tolerance, boundary_condition=condition, in_place=True
    )
    c_2 = bsplines.convert_to_interpolation_coefficients(
        sourceb[..., 0], degree, tolerance, boundary_condition=condition, in_place=True
    )
                ...
                for condition in condition_list:
                    for axis in axes_list:
                        for base in bases_list:
                            from_b, to_b = base
                            test_basis(
                                sig, from_b, to_b, degree, axis, tolerance, condition
                            )
    ...
    def test_dims(self):
        ...
        for degree in range(4):
            for size in data_size_list:
                sig = np.random.default_rng().random(size)
                for condition in condition_list:
                    test_dims_conversion(sig, degree, tolerance, condition)
                    for axis in axes_list:
                        for base in bases_list:
                            from_b, to_b = base
                            test_dims(
    ...
    def test_splines(self):
        ...
        for degree in range(9):
            np.testing.assert_allclose(
                signal.bspline(x, degree), bsplines.bspline(x, degree)
            )
