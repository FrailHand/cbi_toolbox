...
class TestSplradon(unittest.TestCase):
    def test_dimension(self):
        ...
        np.testing.assert_allclose(spline_2d, spline_3d[..., 0], rtol=1e-12, atol=1e-12)

        out_3d = spl.iradon(image_3d)
        out_2d = spl.iradon(image_2d)

        np.testing.assert_allclose(out_2d, out_3d[..., 0], rtol=1e-12, atol=1e-12)
    ...
    def test_contiguous(self):
        ...
        np.testing.assert_allclose(spline_2d, spline_3d[..., 0], rtol=1e-9, atol=1e-12)

        out_3d = spl.iradon(spline_3d)
        out_2d = spl.iradon(spline_2d)

        np.testing.assert_allclose(out_2d, out_3d[..., 0], rtol=1e-9, atol=1e-12)
    ...
    def test_padding(self):
        ...
        for size in range(5, 25):
            ...
            for circle in (True, False):
                rd = spl.radon(image, theta, circle=circle, b_spline_deg=(0, 0))
