...
def opt(obj, psf, theta=None, pad=False):
    ...
    pad : bool, optional
        ...
        (needed if the object is not contained in the inner cylinder to the
    ...
    if not obj.shape[0] == obj.shape[1]:
        raise NotImplementedError("Please provide a square object in ZX dimensions")
    ...
    if psf.shape[0] < full_size:
        raise ValueError(
            "PSF Z size must be greater than padded object Z size: {} >= {}".format(
    ...
    for idx, angle in enumerate(theta):
        rotated = ndimage.rotate(splimage, angle, prefilter=False, reshape=False)
...
def fps_opt(obj, psf, pad=False, **kwargs):
    ...
    pad : bool, optional
        pad the vield of view to see all contributions (required if the sample
        is not contained in the inner cylinder of the object), by default False
...
def spim(obj, psf, illu, pad=False):
    ...
    if psf.shape[0] < illu.shape[0]:
        raise ValueError(
            "PSF depth must be bigger than SPIM illumination depth (Z axis) - {} < {}".format(
                psf.shape[0], illu.shape[0]
            )
        )
    ...
    if pad:
        image = np.empty(
            (
                obj.shape[0] + thickness - 1,
                obj.shape[1] + psf.shape[1] - 1,
                obj.shape[2] + psf.shape[2] - 1,
            )
        )
        mode = "full"
        ...
        image = np.empty((obj.shape[0], obj.shape[1], obj.shape[2]))
        mode = "same"
...
def noise(
    image, photons=200, background=3, max_amp=None, clip=True, seed=None, in_place=False
):
    """
    ...
    max_amp : float, optional
        the value of a pixel corresponding to max photon count, by
        default None (uses the maximum value of the input image)
    clip : bool, optional
        clip the output values to the range [0, max_amp], by default True
    seed : int, optional
    ...
    if clip:
        poisson = poisson.clip(0, photons)

    return max_amp * poisson / photons


def quantize(image, bits=12):
    """
    Perform quantization of a continuous array

    Parameters
    ----------
    image : array (float in [0,1])
        the continuous input array
    bits : int, optional
        the number of bits to use for quantization, by default 12
    ...
    if image.max() > 1 or image.min() < 0:
        raise ValueError("Input values for quantize should be between 0 and 1")

    return np.rint(image * (2**bits - 1)).astype(int)
